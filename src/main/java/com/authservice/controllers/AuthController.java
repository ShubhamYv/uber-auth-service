...
public class AuthController {
    ...
    public ResponseEntity<PassengerDto> signUp(@RequestBody PassengerSignupRequest request) {
        ...
        if (request == null) {
            LogMessage.warn(LOGGER, "Received null signup request");
            return ResponseEntity.badRequest().build();
        }

        LogMessage.info(LOGGER, "Processing passenger signup request: " + request);
        ...
        LogMessage.info(LOGGER, "Passenger signed up successfully: " + passengerDto);
    ...
    public ResponseEntity<String> signIn(@RequestBody PassengerSigninRequest request,
        ...
        if (request == null) {
            LogMessage.warn(LOGGER, "Received null signin request");
            return ResponseEntity.badRequest().build();
        }

        LogMessage.info(LOGGER, "Processing passenger signin request: " + request);
        ...
        LogMessage.info(LOGGER, "Passenger signed in successfully. JWT Token: " + jwtToken);
        ...
        try {
            ...
            if (cookies != null) {
                for (Cookie cookie : cookies) {
                    LogMessage.debug(LOGGER, "Cookie:: name->" + cookie.getName() + " |value-> " + cookie.getValue());
                }
            } else {
                LogMessage.info(LOGGER, "No cookies found in the request");
            }
